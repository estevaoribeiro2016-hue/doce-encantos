...
async function loadPublicInventory(){if(!supabaseClient)return;const [inv,zones,settings]=await Promise.all([supabaseClient.from('inventory').select('*').order('flavor_id'),supabaseClient.from('delivery_zones').select('*').eq('active',true).order('name'),supabaseClient.from('app_settings').select('*').eq('id',1).maybeSingle()]);if(inv.error)throw inv.error;if(zones.error)console.warn(zones.error);if(settings.error)console.warn(settings.error);if(inv.data?.length){inventory=inv.data.map(r=>({id:r.flavor_id,stock:Number(r.stock||0),min:Number(r.min_stock||0)}));products=BASE_PRODUCTS.map(p=>({...p,...(inventory.find(i=>i.id===p.id)||{})}));}deliveryZones=(zones.data||[]).map(z=>({id:z.id,name:z.name,normalizedName:z.normalized_name,fee:Number(z.fee||0),active:!!z.active,latitude:z.latitude,longitude:z.longitude}));if(settings.data){storeSettings={openTime:settings.data.open_time||'09:00',closeTime:settings.data.close_time||'20:00',openDays:settings.data.open_days||[1,2,3,4,5,6],defaultEta:Number(settings.data.default_eta||30),ordersEnabled:settings.data.orders_enabled!==false};}saveLocalOnly();renderStoreStatus();}
async function loadAdminSupabaseState(){if(!supabaseClient)return;const [o,m,z]=await Promise.all([supabaseClient.from('orders').select('*').order('created_at',{ascending:false}).limit(1000),supabaseClient.from('stock_movements').select('*').order('created_at',{ascending:false}).limit(1000),supabaseClient.from('delivery_zones').select('*').order('name')]);if(o.error)throw o.error;if(m.error)throw m.error;if(z.error)throw z.error;orders=(o.data||[]).map(orderFromSupabase);lastKnownOrderIds=new Set(orders.map(x=>x.id));stockMoves=(m.data||[]).map(moveFromSupabase);deliveryZones=(z.data||[]).map(x=>({id:x.id,name:x.name,normalizedName:x.normalized_name,fee:Number(x.fee||0),active:!!x.active,latitude:x.latitude,longitude:x.longitude}));deliveryZonesDraft=deliveryZones.map(x=>({...x}));saveLocalOnly();}
function orderFromSupabase(r){return{id:r.id,created:r.created_label||new Date(r.created_at).toLocaleString('pt-BR'),customerName:r.customer_name,customerPhone:r.customer_phone,items:r.items||[],subtotal:Number(r.subtotal||0),freight:Number(r.freight||0),total:Number(r.total||0),fulfillment:r.fulfillment,deliveryMethod:r.delivery_method,deliveryRegion:r.delivery_region,address:r.address,payment:r.payment,paymentLabel:r.payment_label,status:r.status,stockRestored:!!r.stock_restored,createdAt:r.created_at,readyAt:r.ready_at?new Date(r.ready_at).toLocaleString('pt-BR'):'',deliveredAt:r.delivered_at?new Date(r.delivered_at).toLocaleString('pt-BR'):'',canceledAt:r.canceled_at?new Date(r.canceled_at).toLocaleString('pt-BR'):'',receiptStatus:r.receipt_status||'Aguardando comprovante',receiptReceivedAt:r.receipt_received_at?new Date(r.receipt_received_at).toLocaleString('pt-BR'):'',estimatedMinutes:Number(r.estimated_minutes||storeSettings.defaultEta||30)};}
...
let storeSettings = { openTime: '09:00', closeTime: '20:00', openDays: [1,2,3,4,5,6], defaultEta: 30, ordersEnabled: true };
let lastKnownOrderIds = new Set();
let lastWhatsappOrder = loadJSON('lastWhatsappOrder', null);
...
function buildWhatsappMessage(order) {
  ...
  if (order.payment === 'pix') {
    lines.push('Chave Pix: 31992180872');
    lines.push('Titular: Ingrid Emanuelle Damasceno');
    lines.push('Envie o comprovante por este WhatsApp para confirmar o pagamento.');
  }
  lines.push('');
  lines.push(`⏱️ *Tempo estimado:* aproximadamente ${order.estimatedMinutes||storeSettings.defaultEta||30} minutos`);
  lines.push('🧾 *Comprovante:* envie por este WhatsApp para confirmação');
...
function timeToMinutes(v){const [h,m]=String(v||'00:00').split(':').map(Number);return h*60+(m||0);}
function storeOpenState(){if(!storeSettings.ordersEnabled)return {open:false,message:'Pedidos temporariamente pausados pela loja.'};const d=new Date(),day=d.getDay(),now=minutesNow(),start=timeToMinutes(storeSettings.openTime),end=timeToMinutes(storeSettings.closeTime);if(!storeSettings.openDays.includes(day))return {open:false,message:`Hoje não estamos recebendo pedidos. Horário: ${storeSettings.openTime} às ${storeSettings.closeTime}.`};if(now<start||now>end)return {open:false,message:`Estamos fechados agora. Recebemos pedidos das ${storeSettings.openTime} às ${storeSettings.closeTime}.`};return {open:true,message:`Aberto agora • pedidos até ${storeSettings.closeTime}`};}
function renderStoreStatus(){const el=$('#storeOpenStatus');if(!el)return;const st=storeOpenState();el.textContent=st.open?'🟢 '+st.message:'🔴 '+st.message;el.className='storeOpenStatus '+(st.open?'open':'closed');}
function reopenLastWhatsapp(){if(!lastWhatsappOrder)return alert('Finalize um pedido primeiro.');window.open(`https://wa.me/5531992180872?text=${encodeURIComponent(buildWhatsappMessage(lastWhatsappOrder))}`,'_blank');}
function playNewOrderSound(){try{const C=window.AudioContext||window.webkitAudioContext,ctx=new C(),osc=ctx.createOscillator(),gain=ctx.createGain();osc.connect(gain);gain.connect(ctx.destination);osc.frequency.setValueAtTime(880,ctx.currentTime);gain.gain.setValueAtTime(.18,ctx.currentTime);gain.gain.exponentialRampToValueAtTime(.001,ctx.currentTime+.7);osc.start();osc.stop(ctx.currentTime+.7);}catch(e){console.warn(e);}}
function notifyNewOrder(raw){playNewOrderSound();document.title='🔔 NOVO PEDIDO • Doce Encanto';setTimeout(()=>document.title='Doce Encanto | V56.12',5000);if('Notification'in window&&Notification.permission==='granted')new Notification('Novo pedido Doce Encanto',{body:`Pedido #${raw.id} • ${raw.customer_name||'Cliente'}`});alert(`🔔 Novo pedido recebido!
Pedido #${raw.id}
Cliente: ${raw.customer_name||''}`);}
async function requestNotificationPermission(){if(!('Notification'in window))return alert('Este navegador não oferece notificações.');const p=await Notification.requestPermission();alert(p==='granted'?'Alertas ativados neste navegador.':'Permissão de notificações não concedida.');}
async function finish(){
  if(!supabaseReady)return alert('O banco online ainda não está configurado. Siga o README da V50 REAL.');
  if(!cart.length)return alert('Seu carrinho está vazio.');
  const openState=storeOpenState();if(!openState.open)return alert(openState.message);
  ...
  try{const payload={customerName,customerPhone,items:JSON.parse(JSON.stringify(cart)),fulfillment,address,payment,paymentLabel:normalizePaymentLabel(payment)};const {data,error}=await supabaseClient.rpc('create_order',{p_payload:payload});if(error)throw error;const order=orderFromSupabase(data);orders.unshift(order);lastWhatsappOrder=order;localStorage.setItem(STORE+'lastWhatsappOrder',JSON.stringify(order));cart=[];save();await loadPublicInventory();renderCart();renderProducts();renderPromo();confetti();jump('Pedido salvo! Agora confirme pelo WhatsApp. 💖');const whatsappUrl=`https://wa.me/5531992180872?text=${encodeURIComponent(buildWhatsappMessage(order))}`;window.location.href=whatsappUrl;}
...
async function markReceiptReceived(id){try{const {data,error}=await supabaseClient.rpc('admin_mark_receipt_received',{p_order_id:id});if(error)throw error;await loadAdminSupabaseState();renderAdmin();alert('Comprovante marcado como enviado e recebido.');}catch(e){alert(e.message||'Erro ao confirmar comprovante.');}}
async function updateOrderEta(id,minutes){try{const value=Math.max(5,Math.min(240,Number(minutes)||30));const {error}=await supabaseClient.rpc('admin_set_order_eta',{p_order_id:id,p_minutes:value});if(error)throw error;await loadAdminSupabaseState();renderAdmin();}catch(e){alert(e.message||'Erro ao alterar tempo estimado.');}}
async function saveStoreSettings(){try{const days=$$('[data-open-day]:checked').map(x=>Number(x.value));const payload={open_time:$('#settingOpenTime').value,close_time:$('#settingCloseTime').value,open_days:days,default_eta:Math.max(5,Number($('#settingDefaultEta').value)||30),orders_enabled:$('#settingOrdersEnabled').checked};const {error}=await supabaseClient.rpc('admin_save_app_settings',{p_settings:payload});if(error)throw error;await loadPublicInventory();renderAdmin();alert('Horário e tempo estimado salvos.');}catch(e){alert(e.message||'Erro ao salvar configurações.');}}
function renderPendingOrders(pending) { if (!pending.length) return '<p class="emptyState">Nenhum pedido pendente no momento.</p>'; return pending.map(o => `<article class="orderCard status-${statusBadgeClass(o.status)}"><div class="orderTop"><div><b>#${o.id}</b><small>${o.created}</small></div><span class="pill ${statusBadgeClass(o.status)}">${o.status}</span></div><div class="orderClient"><b>${o.customerName}</b><small>${o.customerPhone}</small></div><div class="orderItemsMini">${orderShortItems(o)}</div><div class="orderMeta"><span>${o.fulfillment === 'entrega' ? '🛵 Entrega' : '🏪 Retirada'}</span><b>${BRL(o.total)}</b></div><div class="orderExtraInfo"><span class="pill ${o.receiptStatus==='Comprovante enviado'?'ok':'warn'}">🧾 ${o.receiptStatus}</span><label>⏱️ Estimativa <input data-eta="${o.id}" type="number" min="5" max="240" value="${o.estimatedMinutes||storeSettings.defaultEta}"> min</label></div><div class="orderActions smartActions"><select data-status="${o.id}"><option ${o.status === 'Recebido' ? 'selected' : ''}>Recebido</option><option ${o.status === 'Produção' ? 'selected' : ''}>Produção</option><option ${o.status === 'Pronto' ? 'selected' : ''}>Pronto</option><option ${o.status === 'Saiu para entrega' ? 'selected' : ''}>Saiu para entrega</option><option ${o.status === 'Aguardando retirada' ? 'selected' : ''}>Aguardando retirada</option><option ${o.status === 'Entregue' ? 'selected' : ''}>Entregue</option></select><button class="secondary" data-next="${o.id}">Avançar</button><button class="primary" data-ready="${o.id}">🟢 Pedido pronto</button>${o.fulfillment === 'entrega' ? `<button class="secondary" data-delivery="${o.id}">🛵 Saiu para entrega</button>` : ''}<button class="secondary" data-receipt="${o.id}">🧾 Comprovante enviado</button><button class="secondary" data-delivered="${o.id}">✅ Entregue</button><button class="ghost" data-chat="${o.id}">💬 Chamar no WhatsApp</button><button class="dangerBtn" data-cancel="${o.id}">Cancelar</button></div></article>`).join(''); }
...
async function init() {
  ...
  $('#reopenWhatsapp')?.addEventListener('click',reopenLastWhatsapp);renderStoreStatus();setInterval(renderStoreStatus,60000);
  $('#copyPix').onclick = async () => { const value=$('#pixCode')?.value||'31992180872'; try { await navigator.clipboard.writeText(value); alert('Chave Pix copiada: 31992180872'); } catch { $('#pixCode')?.select(); document.execCommand('copy'); alert('Chave Pix copiada: 31992180872'); } };
  ...
  // Service worker desativado nesta versão para impedir cache de versões antigas do checkout.
}
...
function renderAdmin(){
 ...
 <div class="tabPanel" data-tab="settings"><section class="adminCard wide"><h3>⚙️ Configurações</h3><div class="settingsGrid"><label>Abre às <input id="settingOpenTime" type="time" value="${storeSettings.openTime}"></label><label>Fecha às <input id="settingCloseTime" type="time" value="${storeSettings.closeTime}"></label><label>Tempo estimado padrão <input id="settingDefaultEta" type="number" min="5" max="240" value="${storeSettings.defaultEta}"> minutos</label><label><input id="settingOrdersEnabled" type="checkbox" ${storeSettings.ordersEnabled?'checked':''}> Receber pedidos</label></div><div class="dayChecks">${[['Dom',0],['Seg',1],['Ter',2],['Qua',3],['Qui',4],['Sex',5],['Sáb',6]].map(([n,v])=>`<label><input data-open-day type="checkbox" value="${v}" ${storeSettings.openDays.includes(v)?'checked':''}> ${n}</label>`).join('')}</div><button id="saveStoreSettings" class="primary">Salvar horário e estimativa</button><button id="enableNotifications" class="secondary">🔔 Ativar alerta de pedido novo</button><hr><label>Largura da mini impressora <select id="paperWidth"><option value="58" ${printPaperWidth===58?'selected':''}>58 mm</option><option value="80" ${printPaperWidth===80?'selected':''}>80 mm</option></select></label><hr><h4>Início oficial</h4><p>Apaga pedidos, faturamento e movimentações de teste. Mantém produtos, estoque atual, usuários e taxas.</p><button id="resetOfficial" class="dangerBtn">Zerar testes e iniciar oficialmente</button></section></div>`;
 $('#adminBack').onclick=()=>location.hash='home';$('#adminLogout').onclick=async()=>{await supabaseClient?.auth.signOut();currentAdmin=null;orders=[];stockMoves=[];$('#adminPanel').classList.add('hidden');$('.login').classList.remove('hidden')};
 $$('[data-tabbtn]').forEach(btn=>btn.onclick=()=>{$$('[data-tabbtn]').forEach(b=>b.classList.remove('active'));btn.classList.add('active');$$('[data-tab]').forEach(p=>p.classList.toggle('active',p.dataset.tab===btn.dataset.tabbtn))});
