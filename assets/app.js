...
const STORE = 'de_v55_';
const LEGACY_STORES = ['de_v54_', 'de_v40_', 'de_v41_', 'de_v42_'];
...
const WHATSAPP_DEFAULTS = {
  businessPhone: '553192180872',
  greeting: 'Olá! Seja bem-vindo(a) à Doce Encanto 🍫💖',
  menu: 'Como podemos ajudar?\n\n1️⃣ Ver cardápio\n2️⃣ Fazer pedido\n3️⃣ Consultar pedido\n4️⃣ Falar com atendente',
  received: 'Olá, {cliente}! 💖\n\nRecebemos seu pedido #{pedido}. Em breve iniciaremos a produção.\n\nDoce Encanto 🍫',
  production: 'Olá, {cliente}! Seu pedido #{pedido} já está em produção. 🍫✨',
  ready: 'Olá, {cliente}! Seu pedido #{pedido} já está pronto! ✅',
  delivery: 'Olá, {cliente}! Seu pedido #{pedido} saiu para entrega. 🛵',
  delivered: 'Olá, {cliente}! Seu pedido #{pedido} foi entregue. Obrigado pela preferência! 💖\n\nComo foi sua experiência? ⭐⭐⭐⭐⭐',
  canceled: 'Olá, {cliente}. Seu pedido #{pedido} foi cancelado. Se precisar, fale com a gente por aqui.'
};
let whatsappSettings = (()=>{try{return {...WHATSAPP_DEFAULTS,...JSON.parse(localStorage.getItem(STORE+'whatsappSettings')||'{}')}}catch{return {...WHATSAPP_DEFAULTS}}})();
function saveWhatsappSettings(){localStorage.setItem(STORE+'whatsappSettings',JSON.stringify(whatsappSettings));}
function cleanBusinessPhone(v){let d=onlyDigits(v||'');if(d && !d.startsWith('55')) d='55'+d;return d;}
function businessPhone(){return cleanBusinessPhone(whatsappSettings.businessPhone||WHATSAPP_DEFAULTS.businessPhone);}
function fillWhatsappTemplate(template,o={}){return String(template||'').replaceAll('{cliente}',o.customerName||'cliente').replaceAll('{pedido}',o.id||'').replaceAll('{total}',BRL(o.total||0));}
function openBusinessWhatsapp(text=''){const phone=businessPhone();if(!phone)return alert('Cadastre o número do WhatsApp Business na central.');window.open(`https://wa.me/${phone}${text?'?text='+encodeURIComponent(text):''}`,'_blank');}
function installWhatsappFloatingButton(){if($('#whatsappFloat'))return;const b=document.createElement('button');b.id='whatsappFloat';b.className='whatsappFloat';b.type='button';b.title='Falar com a Doce Encanto no WhatsApp';b.innerHTML='💬';b.onclick=()=>openBusinessWhatsapp(`${whatsappSettings.greeting}\n\n${whatsappSettings.menu}`);document.body.appendChild(b);}
...
function orderNotifyMessage(o, type) {
 const key = type === 'delivery' ? 'delivery' : type;
 const template = whatsappSettings[key];
 if(template) return fillWhatsappTemplate(template,o);
 return '';
}
...
function renderAdmin(){
 ...
 $('#adminPanel').innerHTML=`<div class="adminHero"><div><p class="tag">Centro de Controle V55</p><h2>Área da Empresa</h2><p>Pedidos, estoque, financeiro mensal, impressão térmica e taxas de entrega online.</p></div><div class="adminTopActions">${supabaseStatusHtml()}<button id="adminBack" class="secondary">Voltar ao site</button><button id="adminLogout" class="ghost">Sair</button></div></div><div class="dashCards"><div><small>Pendentes</small><b>${pending.length}</b></div><div><small>Produção</small><b>${production.length}</b></div><div><small>Estoque baixo</small><b>${low}</b></div><div><small>Faturamento entregue</small><b>${BRL(deliveredRevenue)}</b></div></div><div class="adminTabs"><button class="active" data-tabbtn="pending">📌 Pendentes</button><button data-tabbtn="production">🏭 Produção</button><button data-tabbtn="history">📚 Histórico</button><button data-tabbtn="stock">📦 Estoque</button><button data-tabbtn="moves">🔁 Movimentações</button><button data-tabbtn="finance">💰 Financeiro mensal</button><button data-tabbtn="zones">🛵 Taxas</button><button data-tabbtn="whatsapp">💬 WhatsApp</button><button data-tabbtn="settings">⚙️ Configurações</button></div>
 ...
 <div class="tabPanel" data-tab="whatsapp"><section class="adminCard wide"><h3>💬 WhatsApp Business</h3><p class="helper">Configuração segura para abrir conversas e mensagens prontas. O envio só acontece quando vocês confirmam no WhatsApp.</p><div class="waConfig"><label>Número do WhatsApp Business<input id="waBusinessPhone" inputmode="tel" value="${escapeHtml(whatsappSettings.businessPhone)}" placeholder="Ex.: 31999999999"></label><label>Saudação<textarea id="waGreeting" rows="2">${escapeHtml(whatsappSettings.greeting)}</textarea></label><label>Menu automático<textarea id="waMenu" rows="6">${escapeHtml(whatsappSettings.menu)}</textarea></label><label>Pedido recebido<textarea id="waReceived" rows="4">${escapeHtml(whatsappSettings.received)}</textarea></label><label>Em produção<textarea id="waProduction" rows="3">${escapeHtml(whatsappSettings.production)}</textarea></label><label>Pedido pronto<textarea id="waReady" rows="3">${escapeHtml(whatsappSettings.ready)}</textarea></label><label>Saiu para entrega<textarea id="waDelivery" rows="3">${escapeHtml(whatsappSettings.delivery)}</textarea></label><label>Entregue / avaliação<textarea id="waDelivered" rows="4">${escapeHtml(whatsappSettings.delivered)}</textarea></label><div class="zoneActions"><button id="waSave" class="primary">Salvar WhatsApp</button><button id="waTest" class="secondary">Testar conversa</button><button id="waReset" class="ghost">Restaurar mensagens</button></div><small>Campos disponíveis: {cliente}, {pedido} e {total}. Respostas totalmente automáticas sem confirmação exigem WhatsApp Cloud API e um servidor seguro.</small></div></section></div>
 ...
 $('#waSave')?.addEventListener('click',()=>{whatsappSettings={...whatsappSettings,businessPhone:cleanBusinessPhone($('#waBusinessPhone')?.value),greeting:$('#waGreeting')?.value||'',menu:$('#waMenu')?.value||'',received:$('#waReceived')?.value||'',production:$('#waProduction')?.value||'',ready:$('#waReady')?.value||'',delivery:$('#waDelivery')?.value||'',delivered:$('#waDelivered')?.value||''};saveWhatsappSettings();alert('Configurações do WhatsApp salvas neste navegador.');});
 $('#waTest')?.addEventListener('click',()=>openBusinessWhatsapp(`${$('#waGreeting')?.value||whatsappSettings.greeting}\n\n${$('#waMenu')?.value||whatsappSettings.menu}`));
 $('#waReset')?.addEventListener('click',()=>{if(confirm('Restaurar as mensagens padrão do WhatsApp?')){whatsappSettings={...WHATSAPP_DEFAULTS,businessPhone:whatsappSettings.businessPhone};saveWhatsappSettings();renderAdmin();}});
